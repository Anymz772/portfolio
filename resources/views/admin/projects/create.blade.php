...
<div class="max-w-3xl mx-auto">
    <div class="mb-8">
        <h1 class="text-3xl font-heading font-bold text-white">Add New Project</h1>
        <p class="text-text-secondary mt-2">Add a new project to showcase in your portfolio</p>
    </div>

    <form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data" class="glass-card p-6 rounded-xl space-y-6">
        @csrf
        ...
        <div>
            ...
            <textarea name="long_description" rows="5"
                      class="w-full px-4 py-3 bg-dark-card border border-glass-border rounded-xl focus:border-accent focus:outline-none transition-colors text-white resize-none"
                      placeholder="Detailed description (optional)">{{ old('long_description') }}</textarea>
        </div>
        ...
        <div>
            ...
            <input type="text" name="technologies" value="{{ old('technologies') }}"
                   class="w-full px-4 py-3 bg-dark-card border border-glass-border rounded-xl focus:border-accent focus:outline-none transition-colors text-white"
                   placeholder="e.g., Laravel, MySQL, Tailwind CSS, Vue.js">
        </div>
        ...
        <div>
            ...
            <input type="text" name="features" value="{{ old('features') }}"
                   class="w-full px-4 py-3 bg-dark-card border border-glass-border rounded-xl focus:border-accent focus:outline-none transition-colors text-white"
                   placeholder="e.g., Authentication, Dashboard, Real-time updates">
        </div>
